...
public class AuthController {
    ...
    @PostMapping("/associate-rib")
    @PreAuthorize("hasRole('ADMIN')") // Ajout pour restreindre l'accès aux admins
    public ResponseEntity<?> associateRib(@Valid @RequestBody AssociateRibRequest request) {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        if (auth == null || !auth.isAuthenticated() || !(auth.getPrincipal() instanceof UserDetailsImpl)) {
            return ResponseEntity.status(401).body(new MessageResponse("Utilisateur non authentifié !"));
        }
        ...
        Account existingAccount = accountRepository.findByRib(request.getRib()).orElse(null);
        if (existingAccount == null) {
            return ResponseEntity.badRequest().body(new MessageResponse("Erreur : Le RIB fourni n'existe pas !"));
        }

        User existingUserWithRib = userRepository.findByAccounts_Rib(request.getRib()).orElse(null);
        if (existingUserWithRib != null && !existingUserWithRib.getId().equals(user.getId())) {
            return ResponseEntity.badRequest().body(new MessageResponse("Erreur : RIB déjà associé !"));
        }

        user.addAccount(existingAccount);
        userRepository.save(user);

        return ResponseEntity.ok(new MessageResponse("RIB associé ! Solde : " + existingAccount.getSolde()));
    }
}